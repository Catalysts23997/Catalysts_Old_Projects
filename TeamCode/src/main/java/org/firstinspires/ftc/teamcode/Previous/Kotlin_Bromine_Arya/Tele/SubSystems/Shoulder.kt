...
open class Shoulder(hwMap: HardwareMap) {
    ...
    private var shoulderPIDF = PIDFcontroller(PIDParams(.023, .00008, .001175, .123))
    ...
    fun pidShoulder() {
        findTarget()
        shoulder.power =  if (target == 0.0) { powerDrop } else {shoulderPIDF.calculate(target -shoulder.currentPosition.toDouble())}
    }
